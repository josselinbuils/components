...
export const tooltipPosition = (element, position) => {
  const { top, left, right, bottom, width, height } = element;
  const leftSpace = left + width / 2 + window.pageXOffset;
  ...
  switch (true) {
    ...
    case position === 'left' && left > requiredSpace:
      return {
        ...
        style: {
          top: top + height / 2,
          right: document.body.clientWidth - left + window.pageXOffset + space,
        },
      };
